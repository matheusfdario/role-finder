...
from framework.data_types import ImagingROI, ImagingROI3D, ElementGeometry  # ImagingROI3D nao existe
...
for i_config in range(1,31,2):
    ...
    path = 'G:/Dados 3D/Retangular Circular/RetangularCircularRadius_'+pitch_str+'.civa'
    print(path)
    ...
    chave = tfm3d.tfm3d_kernel(data, roi=roi, sel_shot=0, c=data.specimen_params.cl, elem_geometry=ElementGeometry.CIRCULAR)#, scattering_angle=20)
    ...
    print('Saving data...')
    np.save('./newrectcirc/'+title, data.imaging_results[chave].image)
    plt.show()
    plt.xlabel('x[mm]')
    plt.ylabel('z[mm]')
    plt.title(title)
    print('Saving figure...')
    plt.savefig('./newrectcirc/'+title+'.png')
    plt.close(fig)
    ...
    print('Result ' + (i_config).__str__()+'/30' + '. Elapsed time [s] ' + (time.time() - t0).__str__())
