...
from imaging import bscan, saft, wk_saft, utsr, utsr_fista
...
if full_roi:
    ...
    if using_civa:
        ...
        if simul_data:
            corner_roi = np.array([15.0, 0.0, 18.0])[np.newaxis, :]
            roi = ImagingROI(corner_roi, height=40.0, width=32.0, h_len=683, w_len=31)
...
if simul_data:
    ...
    data_point.ascan_data[idx_t0: idx_te, 0, 0, :] = s_t_u_point + ruido_ascan
...
t.tic()
chave_utsr_fista = utsr_fista.utsr_fista_kernel(data_point, roi=roi, c=cl, debug=True, alpha=alpha/3.0)
t.toc()
print("FISTA executado em %f segundos" % t.elapsed)
image_out_fista = normalize(envelope(data_point.imaging_results[chave_utsr_fista].image, -2))
...
fg5 = plt.figure()
im_utsr_fista = plt.imshow(image_out_fista, aspect='auto', cmap=plt.get_cmap(cmap_string),
                           extent=[roi.w_points[0], roi.w_points[-1], roi.h_points[-1], roi.h_points[0]])
plt.title("FISTA", {'fontsize': 8})
plt.grid(b=True, which='major', linestyle='-')
plt.minorticks_on()
plt.grid(b=True, which='minor', linestyle='--')
plt.show()
