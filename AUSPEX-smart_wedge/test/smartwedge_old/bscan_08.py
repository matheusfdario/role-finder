...
def GenerateImage(shot):
    ...
    # ang = 2
    # plt.plot(t_span, referencia[:, ang] * ganho_4db[ang], label='Wedge com ganho')
    # plt.plot(t_span, np.log10, label='4dB')
    # img_subtraido = inspecao - np.roll(referencia * ganho_4db[ang], 1, axis=0)
    # plt.plot(t_span, img_subtraido[:, ang], label='Subtração')
    # plt.legend()

    #
    # plt.imshow(np.log10(envelope(img_subtraido) + 1), aspect='auto')
    #
    ...
    vmin_cte = img_combined_log.min()
    vmax_cte = img_combined_log.max()
    ...
    plt.imshow(img_combined_log, aspect='auto', interpolation='none', extent=[-92.5, 92.5, t_span[-1], t_span[0]],
               vmin=1, vmax=1.8, cmap=plt.get_cmap("Greys").reversed())
    plt.xlabel("Ângulo de Varredura")
    plt.ylabel("Tempo em $\mu$s")
...
video_title = "VistaSetorial_Combinada_ComReferência_SEAM_Subtração_corrigida"
