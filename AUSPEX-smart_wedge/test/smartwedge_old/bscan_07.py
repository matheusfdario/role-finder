...
def GenerateImage(shot):
    ...
    inspecao =  data[1].ascan_data_sum
    ...
    idx_wedge = np.argmax(referencia[idx0:idxf, :], 0)
    max_insp = np.max(inspecao[idx0:idxf, :], 0)
    idx_insp = np.argmax(referencia[idx0:idxf, :], 0)

    delta_t = np.abs(t_span[idx_wedge] - t_span[idx_insp])

    ganho = max_insp/max_wedge



    # ganho = cte_4db / cte_7db
    ang = 25
    plt.plot(t_span, referencia[:, ang] * ganho[ang], label='Wedge com ganho')
    plt.plot(t_span, inspecao[:, ang], label='4dB')
    resultado = inspecao - np.roll(referencia * ganho, 1, axis=0)
    plt.plot(t_span, resultado[:, ang], label='Subtração')
    plt.legend()

    inspecao_log = np.log10(np.abs(inspecao) + 1)
    vmin_cte = inspecao_log.min()
    vmax_cte = inspecao_log.max()

    plt.subplot(3,1,1)
    plt.imshow(np.log10(np.abs(referencia) + 1), aspect='auto', vmin=vmin_cte, vmax=vmax_cte)
    plt.title("Referência 7 dB")
    plt.subplot(3,1,2)
    plt.imshow(np.log10(np.abs(inspecao) + 1), aspect='auto', vmin=vmin_cte, vmax=vmax_cte)
    plt.title("Ganho 4 dB")
    plt.subplot(3,1,3)
    img_subtraida = np.log10(np.abs(inspecao - np.roll(referencia* ganho, 1, axis=0) ) + 1)
    plt.imshow(img_subtraida, aspect='auto', vmin=vmin_cte, vmax=vmax_cte)
    plt.title("Ganho 4 dB subtraido")
