...
for i_config in range(1,31,2):
    ...
    data = file_civa.read('G:/Retangular Circular/RetangularCircularRadius_'+pitch_str+'.civa')
    ...
    print('Performing 3D TFM scattering_angle...')
    ...
    print('Saving data...')
    np.save('./newrectcirc/' + title + '_Angle12', data.imaging_results[chave].image)
    plt.show()
    plt.xlabel('x[mm]')
    plt.ylabel('z[mm]')
    plt.title(title)
    print('Saving figure...')
    plt.savefig('./newrectcirc/' + title + '_Angle12.png')
    plt.close(fig)
