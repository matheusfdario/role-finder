...
from smartwedge.generate_law import *
import scipy.optimize as optm

from smartwedge.smartwedge import Smartwedge


def find_focus_coord(r_focus, ang_focus_deg_sign, smartwedge):
    ...
    ang_focus_deg = np.abs(ang_focus_deg_sign)
    angle_a = 0
    ...
    return focus_dist, np.rad2deg(focus_angle) * np.sign(ang_focus_deg_sign)
...
if __name__ == "__main__":
    ...
    r_focus = 70 - 10
    ang_focus = np.arange(-40, 40.5, .5)
    focus_dist_vec = list()
    focus_ang_deg_vec = list()
    for ang in ang_focus:
        focus_dist, focus_ang_deg = find_focus_coord(r_focus, ang, sm)
        focus_dist_vec.append(focus_dist)
        focus_ang_deg_vec.append(focus_ang_deg)

    plt.plot(np.array(focus_dist_vec) * 1e3, 'o')
    np.save('focus_dist_vec', focus_dist_vec)
    np.save('focus_ang_deg_vec', focus_ang_deg_vec)
    #
