...
if __name__ == "__main__":
    ...
    step = 2
    betas = np.arange(0, 42 + step, step)

    # # Range em X da região da curva parametrizada:
    xmax = optm.minimize_scalar(sm.parametric_curve, bounds=(0, B[0]), method='bounded').x

    x = np.arange(-r0, r0, 1e-5)
    t = np.arange(xint, xmax, 1e-5)

    # Encapsulamento da curva parametrizada:

    curve = lambda x: sm.parametric_curve(x)
    ...
    b = np.sqrt(a ** 2 - c ** 2)
    ellipse_fun = lambda x: np.sqrt((b ** 2) * (1 - ((x - c) ** 2) / (a ** 2)))

    # Descobrindo o ponto em que a elipse intercepta a circunferÊncia:
    x_tubulacao = np.arange(start=-raio / 2, stop=raio)
    y_tubulacao = f_circle(x_tubulacao)
    y_elipse = ellipse_fun(x_tubulacao)
    x_intercept_elipse_tub = x_tubulacao[np.argmin(np.power(y_tubulacao - y_elipse, 2))]

    # Circunferência que liga a região do transdutor e ellipse:
    cir_r = np.sqrt(B[0] ** 2 + sm.Tprime ** 2)
    # Qual é a coordenada de intersecção:
    search_span = np.arange(0, cir_r, 1e-3)
    cir_eq = lambda x: np.sqrt(cir_r ** 2 - x ** 2)
    x_int_circ_ellipse = search_span[np.argmin(np.power(ellipse_fun(search_span) - cir_eq(search_span), 2))]

    table = PrettyTable(["β_original", "β_calculado", "α1", "α2", "ângulo normal à circunferência (calculado)"])
    for k, beta in enumerate(betas):
        alpha, intersect = sm.compute_entrypoints(beta)
        ...
        if sm.is_indirect_inc([beta]):
            ...
            plt.plot(x_span, an * x_span + bn, ':', color=[0, 1, 0])

            # Reta tangente ao ponto de intersecção (x1,y1):
            tn = np.arange(x1 - 4, x1 + 4)
            an = dfdx
            bn = y1 - dfdx * x1
            # plt.plot(tn, an * tn + bn) # Retas tangentes
            ...
            table.add_row(
                [f"{beta:.2f}", f"{refract_line_angle:.2f}", f"{inc_angle:.2f}", f"{np.rad2deg(refract_angle):.2f}",
                 f"{circle_incid_angle:.2f}"])

            plt.scatter(x1, y1, s=3, color=[1, 0, 0], alpha=0.5)
        else:
            phi = lambda x: np.power(ellipse_fun(x) - planewave_line(x), 2)
            # Encontra o ponto de intersecão da reta que liga o centro do transdutor e a elipse:
            xintersection = optm.minimize_scalar(phi, bounds=(0, x_int_circ_ellipse), method='bounded').x
            # Ponto de intersecção:
            plt.scatter(xintersection, ellipse_fun(xintersection), color=[1, 0, 1])

            # Plota a reta que liga o centro do transdutor e a elipse:
            x_span = np.arange(xintersection, 2 * c)
            plt.plot(x_span, an * x_span + bn, ':', color=[0, 1, 0])

            plt.plot([xintersection, 0], [ellipse_fun(xintersection), 0],  ':', color=[0, 1, 0])

    # Desenha a smartwedge_old:
    sm.draw(plot_tube=True)

    plt.xlabel('Eixo x em milímetros')
    plt.ylabel('Eixo y em milímetros')
    plt.axis('equal')
    plt.xlim([x_intercept_elipse_tub - 20, 170])
    plt.ylim([-130, 130])
    plt.grid(alpha=0.5)
