...
from imaging import wk_cpwc, bscan
...
# Obtém a reconstrução da imagem na ROI com o algoritmo wk-CPWC. 
wk_cpwc_key = wk_cpwc.wk_cpwc_kernel(data, roi)
...
# Exibe o resultado do algoritmo wk-CPWC.
plt.subplot(1, 3, 2)
plt.imshow(data.imaging_results[wk_cpwc_key].image, aspect='auto',
           extent=[roi.w_points[0], roi.w_points[-1], roi.h_points[-1], roi.h_points[0]])
plt.title('$\omega k$-CPWC', fontsize=18)

# Exibe o resultado do algoritmo wk-CPWC com envelope normalizado.
plt.subplot(1, 3, 3)
plt.imshow(normalize(envelope(data.imaging_results[wk_cpwc_key].image, -2)), aspect='auto',
           extent=[roi.w_points[0], roi.w_points[-1], roi.h_points[-1], roi.h_points[0]])
plt.title('$\omega k$-CPWC com pós-processamento', fontsize=18)
