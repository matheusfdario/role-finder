...
from imaging import bscan, utsr_fista
...
plt.subplot(1, 3, 2)
plt.imshow(data_point.imaging_results[chave_utsr_fista].image, aspect='auto',
           extent=[roi.w_points[0], roi.w_points[-1], roi.h_points[-1], roi.h_points[0]])
plt.title('UTSR FISTA', fontsize=18)

# Exibe o resultado do algoritmo UTSR com envelope normalizado.
plt.subplot(1, 3, 3)
plt.imshow(normalize(envelope(data_point.imaging_results[chave_utsr_fista].image, -2)), aspect='auto',
           extent=[roi.w_points[0], roi.w_points[-1], roi.h_points[-1], roi.h_points[0]])
plt.title('UTSR FISTA com pós-processamento', fontsize=18)
