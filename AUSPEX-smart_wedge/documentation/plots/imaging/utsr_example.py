...
from imaging import bscan, utsr
...
plt.subplot(1, 3, 2)
plt.imshow(data_point.imaging_results[chave_utsr].image, aspect='auto',
           extent=[roi.w_points[0], roi.w_points[-1], roi.h_points[-1], roi.h_points[0]])
plt.title('UTSR', fontsize=18)

# Exibe o resultado do algoritmo UTSR com envelope normalizado.
plt.subplot(1, 3, 3)
plt.imshow(normalize(envelope(data_point.imaging_results[chave_utsr].image, -2)), aspect='auto',
           extent=[roi.w_points[0], roi.w_points[-1], roi.h_points[-1], roi.h_points[0]])
plt.title('UTSR com pós-processamento', fontsize=18)
