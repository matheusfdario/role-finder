...
from imaging import wavenumber, bscan
...
# Obtém a reconstrução da imagem na ROI com o algoritmo wavenumber. 
...
# Exibe o resultado do algoritmo wavenumber.
plt.subplot(1, 3, 2)
plt.imshow(data.imaging_results[wavenumber_key].image, aspect='auto',
           extent=[roi.w_points[0], roi.w_points[-1], roi.h_points[-1], roi.h_points[0]])
plt.title('Wavenumber', fontsize=16)

# Exibe o resultado do algoritmo wavenumber com envelope normalizado.
plt.subplot(1, 3, 3)
plt.imshow(normalize(envelope(data.imaging_results[wavenumber_key].image, -2)), aspect='auto',
           extent=[roi.w_points[0], roi.w_points[-1], roi.h_points[-1], roi.h_points[0]])
plt.title('Wavenumber com pós-processamento', fontsize=16)
