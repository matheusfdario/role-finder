...
from imaging import wk_saft, bscan
...
# Obtém a reconstrução da imagem na ROI com o algoritmo wk-SAFT. 
...
# Exibe o resultado do algoritmo wk-SAFT.
plt.subplot(1, 3, 2)
plt.imshow(data.imaging_results[wk_saft_key].image, aspect='auto',
           extent=[roi.w_points[0], roi.w_points[-1], roi.h_points[-1], roi.h_points[0]])
plt.title('$\omega k$-SAFT', fontsize=18)

# Exibe o resultado do algoritmo wk-SAFT com envelope normalizado.
plt.subplot(1, 3, 3)
plt.imshow(normalize(envelope(data.imaging_results[wk_saft_key].image, -2)), aspect='auto',
           extent=[roi.w_points[0], roi.w_points[-1], roi.h_points[-1], roi.h_points[0]])
plt.title('$\omega k$-SAFT com pós-processamento', fontsize=18)
