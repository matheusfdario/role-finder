...
from imaging import e_wavenumber, bscan
...
# Obtém a reconstrução da imagem na ROI com o algoritmo E-wavenumber. 
e_wavenumber_key = e_wavenumber.e_wavenumber_kernel(data, roi)
...
plt.title('B-scan', fontsize=16)

# Exibe o resultado do algoritmo E-wavenumber.
plt.subplot(1, 3, 2)
plt.imshow(data.imaging_results[e_wavenumber_key].image, aspect='auto',
           extent=[roi.w_points[0], roi.w_points[-1], roi.h_points[-1], roi.h_points[0]])
plt.title('E-wavenumber', fontsize=16)

# Exibe o resultado do algoritmo E-wavenumber com envelope normalizado.
plt.subplot(1, 3, 3)
plt.imshow(normalize(envelope(data.imaging_results[e_wavenumber_key].image, 0)), aspect='auto',
           extent=[roi.w_points[0], roi.w_points[-1], roi.h_points[-1], roi.h_points[0]])
plt.title('E-wavenumber com pós-processamento', fontsize=16)
