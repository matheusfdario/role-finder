...
from imaging import vtfm, bscan
...
# Obtém a reconstrução da imagem na ROI com o algoritmo VTFM focando em refletores puntuais.
vtfm_key = vtfm.vtfm_kernel(data, 16, 4, roi, beta=-0.3)
...
# Exibe o resultado do algoritmo VTFM.
plt.subplot(1, 3, 2)
plt.imshow(data.imaging_results[vtfm_key].image, aspect='auto',
           extent=[roi.w_points[0], roi.w_points[-1], roi.h_points[-1], roi.h_points[0]])
plt.title('VTFM', fontsize=18)

# Exibe o resultado do algoritmo VTFM com envelope normalizado.
plt.subplot(1, 3, 3)
plt.imshow(normalize(envelope(data.imaging_results[vtfm_key].image, -2)), aspect='auto',
           extent=[roi.w_points[0], roi.w_points[-1], roi.h_points[-1], roi.h_points[0]])
plt.title('VTFM com pós-processamento', fontsize=18)
